...
#include <string.h>
#include <stdlib.h>
#include <unistd.h> /* kinit-standalone */
...
static int rpc_do_reply(struct client *clnt, struct rpc *rpc, size_t off)
{
	...
	/* kinit-standalone: fixed warnings (comparison casts) */
	if ((ret = read(clnt->sock,
			((char *)rpc->reply) + off,
			rpc->reply_len - off)) == -1) {
		perror("read");
		goto bail;
	} else if (ret < (int) (sizeof(struct rpc_reply) - off)) {
