...
#include <unistd.h>
#include <string.h>
#include <stddef.h> /* kinit-standalone */
...
static int dummy_portmap(int sock, FILE *portmap_file)
{
	...
	struct sockaddr_in sin;
	socklen_t addrlen; /* kinit-standalone: was int */
	ssize_t pktlen; /* kinit-standalone: was int */
	union {
		struct rpc_call rpc;
	...
	for (;;) {
		...
		/* kinit-standalone: add cast (fix warning) */
		if (pktlen + 4 < (ssize_t) sizeof(struct portmap_call))
			continue;	/* Bad packet */
