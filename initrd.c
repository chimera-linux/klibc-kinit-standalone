...
#include <sys/ioctl.h>
#include <sys/wait.h>
#include <linux/mount.h> /* added by kinit-standalone */
#include "do_mounts.h"
#include "kinit.h"
#include "xpio.h"
...
static int run_linuxrc(int argc, char *argv[], dev_t root_dev)
{
	...
	/* klibc-standalone: format fix */
	fprintf(fp, "%lu", (long unsigned int)root_dev);
	fclose(fp);
	...
	/* kinit-standalone: warning fix (cast to dev_t) */
	if ((dev_t)realroot == Root_RAM0) {
