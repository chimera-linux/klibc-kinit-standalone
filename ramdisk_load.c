...
#include <inttypes.h>
#include <sys/stat.h>
#include <sys/ioctl.h> /* added by kinit-standalone */
#include <linux/fs.h>
...
/* klibc-standalone */
#define _fwrite(p, n, f) fwrite(p, 1, n, f)
#define _fread(p, n, f) fread(p, 1, n, f)
			    ...
			    off_t ramdisk_start)
{
	...
	do {
		...
		if (zs.avail_out == 0) {
			...
			/* kinit-standalone: cast to off_t (warning) */
			if (ramdisk_start >= (off_t)ramdisk_size) {
